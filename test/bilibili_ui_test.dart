...
void main() {
  ...
  testWidgets(
    'Bilibili player exposes pages quality MV and queue in landscape',
    (tester) async {
      for (final size in const [Size(640, 360), Size(1280, 800)]) {
        ...
        expect(
          find.byKey(const ValueKey('bilibili-quality-action')).hitTestable(),
          findsOneWidget,
        );
        expect(find.byKey(const ValueKey('player-mv-action')), findsOneWidget);
        expect(find.text('队列'), findsOneWidget);

        await tester.tap(find.byKey(const ValueKey('bilibili-page-102')));
        await tester.pump();
