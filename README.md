...
### 账号与云同步
- 应用功能由账号登录门禁保护，管理员可在独立后台创建、启用、禁用用户和重置密码。
- 登录后自动同步收藏歌曲、收藏歌单、播放历史、搜索历史和个性化设置；离线修改会在恢复网络后继续提交。
- 同一设备上的不同账号使用隔离的本地资料，API Key、B站凭据、WebDAV 密码和缓存不会上传云端。
- 账号服务、PostgreSQL schema、Docker Compose 和部署说明位于 `backend/`。
